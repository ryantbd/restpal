...
var tv4 = require('./tv4-extended');

tv4.addFormat('string-equals', function (data, schema) {
  if (typeof data === 'string' && data.toString() === schema.toString()) {
    return null;
  }

  return 'should be exact same strings';
});
...
Handler.prototype.checkStatus = function (resCode) {
  ...
  if (expected && Number(expected) !== Number(resCode)) {
    this.errors.push({
      name: 'status',
...
Handler.prototype.checkTimer = function () {
  ...
  var expected = Number(this.expects.timer);
  var spent = end - start;
...
Handler.prototype.handleCallbacks = function (res) {
  if (this.callbacks && typeof this.callbacks.check === 'function') {
    try {
      this.callbacks.check(res); 
    } catch (e) {
      this.errors.push({
        name: 'check',
